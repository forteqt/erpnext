...
def get_gl_entries(filters, accounting_dimensions):
		...
		if filters.get("add_values_in_transaction_currency"):
				transaction_currency_fields = "gle.debit_in_transaction_currency, gle.credit_in_transaction_currency, gle.transaction_currency,"
		...
		select
			...
			gle.against, gle.is_opening, gle.creation,
			CASE 
				WHEN gle.voucher_type = 'Journal Entry' 
				THEN je.from_template 
				ELSE NULL 
			END as from_template
			{select_fields}
		...
		if filters.get("include_default_book_entries"):
				if filters.get("finance_book"):
						if filters.get("company_fb") and cstr(filters.get("finance_book")) != cstr(
								...
								conditions.append(
										"(gle.finance_book in (%(finance_book)s, '') OR gle.finance_book IS NULL)"
								)
				else:
						conditions.append(
								"(gle.finance_book in (%(company_fb)s, '') OR gle.finance_book IS NULL)"
						)
		else:
				if filters.get("finance_book"):
						conditions.append(
...
def get_data_with_opening_closing(
		...
		# Modify opening balance presentation
		...
		if opening_balance > 0:
				...
				totals.opening.credit = -opening_balance
		totals.opening.balance = opening_balance
		...
		if filters.get("group_by") != "Group by Voucher (Consolidated)":
				for acc, acc_dict in gle_map.items():
						...
						if acc_dict.entries:
								...
								if filters.get("group_by") != "Group by Voucher":
										acc_opening_balance = (
												acc_dict.totals.opening.debit - acc_dict.totals.opening.credit
										)
										if acc_opening_balance > 0:
												acc_dict.totals.opening.debit = acc_opening_balance
												acc_dict.totals.opening.credit = 0
										else:
												acc_dict.totals.opening.debit = 0
												acc_dict.totals.opening.credit = -acc_opening_balance
										acc_dict.totals.opening.balance = acc_opening_balance
								...
								# closing
								if filters.get("group_by") != "Group by Voucher":
										acc_dict.totals.closing.debit = (
												acc_dict.totals.opening.debit + acc_dict.totals.total.debit
										)
										acc_dict.totals.closing.credit = (
												acc_dict.totals.opening.credit + acc_dict.totals.total.credit
										)
										acc_dict.totals.closing.balance = (
												acc_dict.totals.opening.balance
												+ (acc_dict.totals.total.debit - acc_dict.totals.total.credit)
										)
		...
		totals.closing.balance = totals.opening.balance + (
				totals.total.debit - totals.total.credit
		)
